...
![schema](/img/hinst-for-computer-system-design/lampson.png)
...
-  _Do one thing at a time, and do it well_
GNU утилиты прекрасно иллюстрируют этот принцип. Одна утилита делает одну задачу хорошо и быстро. Из современного мира. Интересный пример про Tenex: каждая часть системы спроектирована хорошо и делает свою работу быстро. Но при совместной работе начинаются задержки, что позволило подбирать пароли, исходя из времени ожижания сообщения об ошибке.
...
- _Don’t hide power_: Не скрывай полезные возможности нижнего уровня. Абстракции скрывают детали реализации, но не должны скрывать стоимость операций.
...
Примеры “place to stand” сегодня:

* возможность зайти по SSH на ноду Kubernetes;
* etcdctl snapshot;
* PostgreSQL physical replication без Patroni;
* аварийный режим восстановления;
* совместимость старых API.

Когда всё остальное ломается, остаётся фундамент, на который можно опереться
...
- _Dynamic translation_ from a convenient (compact, easily modified or easily displayed) representation to one that can be quickly interpreted is an important variation on the old idea of compiling
    - Примеры: JIT переводит программу в байт-код каждый раз при исполнении, regex сначала компилируется, потом выполняется
...
- _Safety first_: чрезмерное потребление ресурсов может привести к проблемам, поэтому при управлении ресурсами важнее оставить запас и избежать катастрофического режима, чем добиться максимальной утилизации
    - Использование >2/3 может снижать производительность системы: ПРИМЕРЫ С ФС, МЕМОРИ
...
- _End-to-end_: проверить правильность операции может только само приложение
    ...
    - Не-а! По пути могло быть много ошибок. Банально, файл мог быть подменен хаекром по пути
